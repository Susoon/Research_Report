# Report for 20.05 ~ 20.12

---
## 12/29 현재상황

* 논문 주제 관련 조사를 진행중에 있다.
* 조사 중인 내용들은 다음과 같다

1. client\(front\-end server\)에서 user가 보낸 query의 key값을 DB에 저장되어 있는 key 값으로 hash할때 사용되는 hash function이 무엇인가
2. UVM의 성능 저하와 관련된 issue 및 solution
3. Mega\-KV의 경우 AES 등의 hash function을 사용하였는데, 논문 작성을 위해 사용할 새로운 hash function은 어떤 것이 있는가 or 어떤 과정으로 hash function이 만들어지는가
4. Mega\-KV와 달리 NIC\-GPU 간의 Direct communication으로 얻을 수 있는 성능적 이점이 어느정도로 확실한가
5. memcached를 사용하는 경우 한 노드당 사용하는 메모리의 크기는 어느정도가 되는가
6. memcached의 real trace가 있는가 or synthetic data는 어떤 것을 사용하는가
7. memcached / Redis / MICA의 특징, 구조 및 각 장단점은 어떻게 되는가
...
* DoDo의 아이디어로 **UVM의 사용**이 있다.
* UVM을 사용하게 되면 GPU에서 Host의 DRAM에 대한 접근 및 관리 권한을 얻을 수 있을 것으로 보인다.
* 이는 추가적인 조사가 필요해보인다.
...
* 찬규형과 얘기해본 결과 다음의 내용을 파악했다.
1. 현재 찬규형과 동민이형은 ixgbe와 ixgbevf 코드를 파악중에 있다.
2. 가장 중점적으로 확인하는 부분은 **NIC과 ixgbevf 드라이버 간의 연결**이다.
    * 연결시키는 방법이 아닌 어떻게 연결이 되었는가이다.
3. host의 커널 상에 ixgbe와 ixgbevf 드라이버 모두 띄운 채로 packet을 전송한 실험을 진행했다.
    * 그 결과 ixgbe로 모든 패킷이 전송되고 ixgbevf는 VF interface를 하나 잡고 있는 상태로 패킷은 받지 못한다.
    * 자세한 세부 내용은 찬규형 github 참고
4. 여기서 다음 step으로 확인할 것은 NIC이 ixgbevf에 어떠한 방식으로 interrupt를 전달하는 가이다.
    * 이 부분에서 문제가 있어 3번에서 ixgbevf로 패킷이 도달하지 못한 것으로 추정된다.
5. 내가 가장 먼저 해야할 일은 RTX3090이 붙어있는 우랩 서버에 GPU-Ether를 돌려보는 것이다.
    * RTX3090과 호환되는지 확인하는 것이다.
    * 이를 위해서 우랩 서버의 커널 버전을 4.18.15로 낮출 필요가 있다.

---

* hulkbuster 서버에 GPU를 옮기고 QEMU를 실행했다.
* 실행 명령어는 다음과 같다.
```
sudo qemu-system-x86_64 -m 2048 -boot d -hda ubuntu.qcow2 -smp 2 -cpu host -enable-kvm
```
* 아직 각각의 옵션들 중 어떤걸 넣어야 실행인지에 대한 정보가 명확하지 않다.
* 해당 옵션들에 대한 정보를 파악할 필요가 있다.

---

* QEMU가 실행되다가 종료되는 이유를 발견했다.
* `/var/log/syslog`를 확인하니 알 수 있었다.
* 그 이유는 **OOM**때문이었다.
    * OOM은 Out Of Memory였다.
    * ~~그냥 OOM이 뭔지만 검색해봤어도....~~
* 이 전까지 QEMU를 실행시킨 서버는 suhwan 서버였다.
* 그냥 단순하게 **suhwan 서버에 메모리 용량이 부족해서** OOM이 발생했다.
* 현재는 hulkbuster 서버에서 실행중이며, 이전에 suhwan 서버에서 실행시 굉장히 렉이 걸렸었는데 이 부분도 해소된 채로 잘 돌아간다...
* hulkbuster 서버에서 작업을 진행하기 위해서 115.145.179.16의 고정 IP를 hulkbuster에 할당했고, 그 위에 올라갈 VM에는 115.145.179.19의 IP를 할당할 예정이다.
