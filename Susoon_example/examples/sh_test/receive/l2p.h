...
pg_raw_dump_l2p(l2p_t *l2p)
{
	...
	for (i = 0; i < RTE_MAX_ETHPORTS; i++) {
		...
		if (pobj->nb_lids == 0)
			continue;
		printf("  Lids idx %2d: RX %2d, TX %2d, priv %p: ",
 ...
 */
static __inline__ void
pg_set_port_priv(l2p_t *l2p, uint16_t pid, void *ptr)
{
	pobj_t    *pobj = &l2p->ports[pid];
...
/**************************************************************************//**
 * Grab the priv pointer.
 *
 */
static __inline__ void *
get_port_priv(l2p_t *l2p, uint16_t pid)
{
	pobj_t    *pobj = &l2p->ports[pid];
 ...
 */
static __inline__ void
pg_set_lcore_priv(l2p_t *l2p, uint16_t lid, void *ptr)
{
	lobj_t    *lobj = &l2p->lcores[lid];
 ...
 */
static __inline__ void *
get_lcore_priv(l2p_t *l2p, uint16_t lid)
{
	lobj_t    *lobj = &l2p->lcores[lid];
