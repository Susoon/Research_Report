...
//SHKIM, ipsec_256 ver. 20.03.12
__global__ void nf_ipsec_256(struct pkt_buf *p_buf, int* pkt_cnt, unsigned int* ctr, unsigned char* d_nounce, unsigned int* d_key, unsigned char* d_sbox, unsigned char* d_GF2, int chain_seq, unsigned int* seq, unsigned int* extended)
{
	// <<< 8, 960 >>> threads. 
	//	15 threads for 1 pkt. (252B pkt)
	// 960 / 15 = 64, 1TB has 960 threads each and manages 64 pkts.
	unsigned int tid = blockDim.x * blockIdx.x + threadIdx.x;
	unsigned int cur_tid = threadIdx.x / AES_T_NUM;
	// 8 x 960 = 7,680
	// tid : 0 - 7,679 (7,680 threads)
