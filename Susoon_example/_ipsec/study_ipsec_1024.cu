...
{
	// <<< 16, 1008 >>> threads. 
	//	63 threads for 1 pkt. (1510B pkt)
	// 1008 / 64 = 16, 1TB has 1008 threads each and manages 16 pkts.
	unsigned int tid = blockDim.x * blockIdx.x + threadIdx.x;
	unsigned int cur_tid = threadIdx.x / AES_T_NUM;
	// 16 x 1008 = 16,128
	// tid : 0 - 16,127 (16,128 threads)
	...
	// rot_index (0 - 1): {0 x 256(0) ~ 1 x 256(256)} + 256  == 0 ~ 511
	// IV : 16 * 16 =  256
	// aes_tmp : 16 * 16 * 63 = 16,128
	// ictx : 24 * 16 = 384
	// octx : 24 * 16 = 384
	// rot_index : 1
	//-------------------------- Total __shared__ mem Usage : 17,153 / 49,152 (48KB per TB)
	...
	while(true){ // Persistent Kernel (for every threads)
		...
		if(tid < 128*AES_T_NUM){ // Beyond this idx could exceed "rx_buf" boundary.
			//-------------------------- Multi threads Job --------------------------------------------
			if(readNoCache(&p_buf->rx_buf_idx[tid/AES_T_NUM + rot_index*256]) == chain_seq){
				...
				if(tid % AES_T_NUM == 0){ 
					...
					int i;
					for(i = 1; i <= 6; i++)
						p_buf->rx_buf[0x1000 * (tid/AES_T_NUM + rot_index*256) + i] = 0; // padding
					
					p_buf->rx_buf[0x1000 * (tid/AES_T_NUM + rot_index*256) + 1510 + 6] = 6; // padlen 

					p_buf->rx_buf[0x1000 * (tid/AES_T_NUM + rot_index*256) + 1510 + 6 + 1] = IPPROTO_IPIP; // next-hdr (Meaning "IP within IP)
				...
				for(int i = 0; i < 16; i++){
					aes_tmp[cur_tid][((tid%AES_T_NUM)*16) + i] = p_buf->rx_buf[(0x1000 * (tid/AES_T_NUM + rot_index*256)) + sizeof(struct ethhdr) + ((tid%AES_T_NUM) * 16) + i] ^ IV[cur_tid][i];
				}
				for(int i = 0; i < 16; i++){
					p_buf->rx_buf[(0x1000 * (tid/AES_T_NUM + rot_index*256)) + sizeof(struct ethhdr) + sizeof(struct iphdr) + sizeof(struct esphdr) + ((tid%AES_T_NUM) * 16) + i] = aes_tmp[cur_tid][((tid%AES_T_NUM)*16) + i]; 
				}
				__syncthreads();
#if 1
				//-------------------------- Single threads Job --------------------------------------------
				if(tid % AES_T_NUM == 0){
					//////////// Proto_type = ESP set! ///////////
					p_buf->rx_buf[0x1000 * (tid/AES_T_NUM + rot_index*256) + sizeof(struct ethhdr) + 9] = IPPROTO_ESP; // IPPROTO_ESP = 50
					struct ethhdr* ethh;
					struct iphdr* iph;
					struct esphdr* esph;

					ethh = (struct ethhdr *)&p_buf->rx_buf[0x1000 * (tid/AES_T_NUM + rot_index*256)];
				...
				__syncthreads();
					...
					sha1_kernel_global_1024(&p_buf->rx_buf[(0x1000 * (tid/AES_T_NUM + rot_index*256)) + sizeof(struct ethhdr) + sizeof(struct iphdr)], &ictx[cur_tid], extended, 64, (tid/AES_T_NUM + rot_index*256));
					/**** Outer Digest ****/
					// H(K XOR opad, H(K XOR ipad, text)) : 20 Bytes
					sha1_kernel_global_1024(&(ictx[cur_tid].c_state[0]), &octx[cur_tid], extended, 20, (tid/AES_T_NUM + rot_index*256));
			
				//-------------------------- Single threads Job --------------------------------------------
				if(tid % AES_T_NUM == 0){
					atomicAdd(&pkt_cnt[1], 1);	
					p_buf->rx_buf_idx[tid/AES_T_NUM + rot_index*256] = chain_seq+1;
